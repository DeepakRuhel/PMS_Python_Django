//.....##############################.....Stage module js code........#########################################
    ...
    debugger;
        ...
        if (selectedRowData) {
            ...
            $('#EStageName').val(selectedRowData.name);
            $('#EinOrder').val(selectedRowData.inOrder);
            $('#EStageGroup').val(selectedRowData.stageGroupId);
            $('#EDescription').val(selectedRowData.descriptions);
 ...
 // Function to format date to YYYY-MM-DD
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        gridOptions = {
            columnDefs: [
                    ...
                    { headerName: 'Short Order', field: 'inOrder' },
    ...
    function addStage(e){
        ...
        const payload = {
            "name":$('#AStageName').val(),
            "stageGroupId":$('#AStageGroup').val(),
            "descriptions":$('#ADescription').val(),
    ...
    $('#editStageForm').submit(function (e) {
         debugger;
        //Check if the form is valid
        if (!this.checkValidity()) {
            e.preventDefault(); // Prevent form submission if it's not valid
    ...
    function editStage(e){
        ...
        const payload = {
            ...
            "name": $('#EStageName').val(),
            "descriptions": $('#EDescription').val(),
            "inOrder": $('#EinOrder').val(),
            "stageGroupId": $('#EStageGroup').val(),
...
function fetchControlTypeDropdown(selectId, selectedCode) {
    ...
    fetch('/pms/LookupData/?type=STGSTS', options)
        .then(response => {
            if (!response.ok) {
                throw new Error('Network response was not ok');
            }
...
fetchControlTypeDropdown('AStageGroup', null);
fetchControlTypeDropdown('EStageGroup', $('#EStageGroup').val());
